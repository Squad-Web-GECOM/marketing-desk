...
const DeskBooking: React.FC = () => {
  ...
  const loadDesks = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('desks').select('*').eq('is_active', true).order('number');
  ...
  const loadReservations = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('reservations').select('*').eq('date', selectedDate).is('canceled_at', null);
      if (error) throw error;
  ...
  const cancelReservation = async (reservationId: number, deskNumber: number) => {
    ...
    try {
      ...
      }).eq('id', reservationId).eq('user_id', currentUser.id); // Security: only allow canceling own reservations
  ...
  return <>
          ...
          <div className="container mx-auto px-4">
            <div className="d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex align-items-center gap-3">
                ...
                <Button variant="ghost" size="sm" onClick={handleLogout} className="text-brand-light hover:bg-brand-light/10">
                  <LogOut size={16} />
                </Button>
              </div>
            </div>
      ...
      <div className="container mx-auto px-4 py-6">
        {/* Message Alert */}
        {message && <Alert variant={message.type === 'error' ? 'destructive' : 'default'} className="mb-4">
        ...
        <Card className="mb-6 border-0 shadow-sm">
          ...
          <CardContent className="p-4">
            <div className="row">
              ...
              <div className="col-12">
                <div className="d-flex flex-wrap gap-2">
                  {availableDates.map(date => <Button key={date} variant={selectedDate === date ? 'default' : 'outline'} size="sm" onClick={() => setSelectedDate(date)} className="mb-2 h-full flex flex-col items-center px-2 py-3 min-w-[80px] text-center">
                      ...
                      {isToday(date) && <Badge variant="secondary" className="mt-1 px-1 py-0 text-xs">
                          Hoje
                        </Badge>}
                    </Button>)}
                </div>
            ...
            {selectedDate && <div className="mt-3 p-3 bg-primary/5 rounded border-l-4 border-primary">
        ...
        {/* Desk Grid */}
        {selectedDate && <Card className="border-0 shadow-sm">
            <CardHeader className="bg-secondary/20 rounded-t-lg">
              <CardTitle className="d-flex align-items-center text-lg">
            ...
            <CardContent className="p-4">
              {loading ? <div className="text-center py-5">
                ...
                return <div key={desk.id} className="col-lg-3 col-md-4 col-sm-6 mb-4">
                        <Card className={`h-100 position-relative ${isMyReservation ? 'border-success bg-success/5' : isOccupied ? 'border-muted bg-muted/5' : canBook ? 'border-primary bg-primary/5 hover:shadow-md transition-shadow' : 'border-muted bg-muted/5'}`}>
                          <CardHeader className="pb-2">
                            <div className="d-flex justify-content-between align-items-center">
                              <CardTitle className="h5 mb-0">
                                Mesa {desk.number}
                              </CardTitle>
                              <Badge variant={isMyReservation ? 'default' : isOccupied ? 'secondary' : 'outline'} className={isMyReservation ? 'bg-success text-success-foreground' : isOccupied ? 'bg-muted text-muted-foreground' : 'border-primary text-primary'}>
                                {isMyReservation ? 'Minha Reserva' : isOccupied ? 'Ocupada' : 'Livre'}
                              </Badge>
                            </div>
                          </CardHeader>
                          <CardContent className="pt-0">
                            {isOccupied && <p className="text-sm text-muted-foreground mb-3">
                                <User size={14} className="mr-1" />
                                {reservation.user_name}
                              </p>}
                            
                            <div className="mt-auto">
                              {isMyReservation ? <Button variant="destructive" size="sm" onClick={() => cancelReservation(reservation.id, desk.number)} disabled={loading} className="w-100">
                                  {loading ? 'Cancelando...' : 'Cancelar'}
                                </Button> : canBook ? <Button variant="default" size="sm" onClick={() => makeReservation(desk.number)} disabled={loading} className="w-100">
                                  {loading ? 'Reservando...' : 'Reservar'}
                                </Button> : isOccupied ? <Button variant="outline" size="sm" disabled className="w-100">
                                  Ocupada
                                </Button> : <Button variant="outline" size="sm" disabled className="w-100">
                                  Não disponível
                                </Button>}
                            </div>
                          </CardContent>
              ...
              {!loading && desks.length === 0 && <div className="text-center py-5">
